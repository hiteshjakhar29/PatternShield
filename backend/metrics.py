...
if importlib.util.find_spec("prometheus_client"):
    ...
    REQUEST_COUNT = Counter(
        "patternshield_requests_total", "API request count", ["endpoint", "method", "status"], registry=registry
    )
    REQUEST_LATENCY = Histogram(
        "patternshield_request_duration_seconds", "Request duration", ["endpoint"], registry=registry
    )
